...
**Use Activity Heartbeats** so an interrupted Activity resumes from its last recorded progress instead of restarting. This is the Cloud Run equivalent of Lambda's timeout tuning: the same problem — work cut off partway — reached by a completely different mechanism, and fixed in the application rather than in configuration.
...
**`max_count` defaults to 30 instances**, set in the version's Scaling and Lifecycle settings. A pool that stops growing under backlog is either at `max_count` or at a regional Cloud Run quota. <!-- docs/troubleshooting/serverless-workers/cloud-run.mdx:130-138 -->
