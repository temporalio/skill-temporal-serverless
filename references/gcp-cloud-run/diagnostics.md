...
**It starts no instance and does not exercise `run.workerPools.update`.** An invoker with read but not update permission passes validation and then never scales the pool. If validation succeeds but `lastModifier` never changes, **check the update permission** — that is the signature. <!-- docs/troubleshooting/serverless-workers/cloud-run.mdx:94-99 -->
...
Tasks route only to the current version, and a CLI-created version is not current automatically. Verify with `temporal worker deployment describe`.
...
The server creates the binding when a Worker running that version connects and polls. **No Task Queues listed means no Worker has ever polled successfully under this version** — and with nothing bound, the WCI has nothing to watch, so it will never scale.

Unlike Lambda, nothing at registration starts an instance, so an unbound Task Queue here is expected until the pool first runs. To prove an instance *can* start and connect, **run the image locally against the same Namespace**, or read pool logs from a run when an instance was up.

### 4. Is the pool at its ceiling?

If instances are running but the count stops growing while backlog builds:

- **`max_count` defaults to 30.** Raise it in the version's Scaling and Lifecycle settings.
- If the count stalls *below* `max_count`, check the project's [Cloud Run quotas](https://cloud.google.com/run/quotas) for that region. Cloud Run caps instances and CPU per region regardless of what the WCI requests.
...
This is expected behavior, not a misconfiguration. The fix is **Activity Heartbeats**, so a retry resumes from the last recorded progress. → `constraints.md`.
