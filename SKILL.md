...
3. **Author the Worker.** *Install the SDK's serverless Worker package before writing any code* — it is usually shipped separately from the main SDK, either on its own version line (Go, TypeScript) or in lockstep with it (Java), so having the base SDK installed does not mean it is importable. Then read the installed package's actual API surface and write against that; these are Public Preview APIs that drift between versions, and generating code from memory costs a build cycle. **This is not a precaution, it is load-bearing:** as of `temporal-aws-lambda` 1.38.0 the published Java entry point (`LambdaWorker.run`) does not exist in the artifact at all — the real one is `LambdaWorker.define`. Code written from the docs would not compile. Inspect first, in every language. Every Workflow must declare a versioning behavior (`Pinned` or `AutoUpgrade`), per-Workflow or as a Worker-level default — code without it fails at runtime. → `references/sdk-configuration.md` (package, install, entry point, tuned defaults) and `references/<provider>/setup.md` (install commands, API-inspection recipes, handler shape).
